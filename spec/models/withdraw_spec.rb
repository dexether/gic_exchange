  ...
  context 'coin withdraw' do
    describe '#audit!' do
      ...
      it "should be rejected if address is invalid" do
        CoinAPI.stubs(:[]).returns(mock('rpc', validateaddress: {isvalid: false}))
        subject.audit!
        subject.should be_rejected
      end

      it "should be rejected if address belongs to hot wallet" do
        CoinAPI.stubs(:[]).returns(mock('rpc', validateaddress: {isvalid: true, ismine: true}))
        subject.audit!
        subject.should be_rejected
      end

      it "should accept withdraw with clean history" do
        CoinAPI.stubs(:[]).returns(mock('rpc', validateaddress: {isvalid: true}))
        subject.audit!
        subject.should be_accepted
      end
