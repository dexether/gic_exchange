class Withdraw < ActiveRecord::Base
  ...
  aasm :whiny_transitions => false do
    ...
    state :submitted,   after_commit: [:send_email, :notify_admin]
  ...
  def notify_admin
    SystemMailer.withdraw_submitted(self.id, self.currency_obj.key, self.amount).deliver
  end
  ...
  def send_coins!
    if coin?
      ...
      AMQPQueue.enqueue(:withdraw_stripe, id: id)
    end
  end
